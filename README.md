...
新版 PC 界面在视口宽度达到 1024px 时，沿用旧界面的 `Segoe UI / Microsoft YaHei` 字体，使用近白背景、白色工作区和紧凑统计栏。桌面样式独立放在 `frontend/src/desktop.css`；小于 1024px 的手机和平板视口沿用原样式。

PC 内容最大宽度为 1600px，较窄屏幕两侧保留 32px。1280px 以上采用“月历＋340px 选中日详情”双栏；1024–1279px 使用上下排列。点击 PC 日历日期会更新所选日的打卡、净工时和下班预测，再通过“编辑所选日期”打开原编辑弹窗；手机保留点击日期直接编辑。PC 日历按月份显示实际所需的 4、5 或 6 行，趋势与设置位于主工作区下方。

PC 顶部主题按钮依次切换“冷色 → 青绿 → 经典绿”。经典绿保留旧版浅灰绿背景和绿色强调色，在白色详情区突出显示下班预测；主题随备份保存。小于 1024px 时，经典绿暂显示为原有冷色；缩放窗口不会改写主题，回到 PC 宽度会恢复经典绿。手机端仍只在冷色与青绿之间切换。
