...
本地后端与数据库持久化已于 2026-07-13 完成：前端不再使用浏览器存储保存正式数据，FastAPI、SQLite、version 1/2 备份迁移、节假日缓存和单文件前端托管已经集成。当前验证基线为后端 78 项测试、前端 10 项测试、TypeScript/Vite 构建及真实浏览器持久化流程全部通过；节假日下载由后端直连，不读取系统代理环境变量。
