...
新版 PC 界面沿用旧界面的 `Segoe UI / Microsoft YaHei` 字体，使用近白背景、白色工作区和紧凑统计栏。桌面样式独立放在 `frontend/src/desktop.css`。

PC 内容最大宽度为 1600px，较窄屏幕两侧保留 32px。1280px 以上采用“月历＋340px 选中日详情”双栏；1024–1279px 使用上下排列。点击 PC 日历日期会更新所选日的打卡、净工时和下班预测，再通过“编辑所选日期”打开原编辑弹窗。PC 日历按月份显示实际所需的 4、5 或 6 行，趋势与设置位于主工作区下方。

PC 顶部风格按钮显示当前选择。点击打开下拉菜单，再点击选项应用；打开菜单或再次选择当前风格不会写入设置。菜单支持方向键、Home/End、Enter/空格选择、Esc 关闭及 Tab 离开，点击外部也可关闭。选择随数据库和 v3 备份保存，刷新后保留；保存失败时继续使用原风格。

可选风格共六种：原有冷色、青绿、经典绿，以及晴空糖果（冰蓝与糖果色卡片）、星际软糖（深蓝星空与青粉色面板）、奶油手账（奶白纸张、胶带和便签）。新风格覆盖统计栏、日历、详情、图表、编辑弹窗、时间选择器和菜单。字体继续采用 `Segoe UI / Microsoft YaHei`，窗口缩小时仍使用 PC 布局。轻微按钮动效遵循系统“减少动态效果”设置。

更新代码后需重启 Python 服务，使主题接口与前端构建产物同时生效。已有 v1/v2/v3 备份仍可导入。较旧的 PC 版本和冻结的手机归档不识别新主题；如需向它们迁移数据，请先切回冷色、青绿或经典绿再导出。手机归档分支不随本次 UI 更新改变。
...
新版 PC 日历显示**杭州（浙江）**从今天起、含今天的 7 天预报，按杭州时间（UTC+8）对应日期。天气区位于日期格子右上方，使用 32px 彩色图标。宽格显示最低／最高摄氏温度、日均相对湿度和美标 AQI；较窄的格子显示图标和温度，悬停可查看湿度、AQI 和统计口径。过去日期和预报范围外不显示天气。旧版 PC 界面保留现有功能。

天气与空气质量通过两个独立请求加载；空气质量较慢或失败不会阻塞天气，二者均不影响打卡、请假或工时计算。服务端缓存一小时，杭州时间换日后重新获取；页面在前台每小时及换日时刷新，重新回到页面时检查是否需要更新。更新失败时仅显示仍在预报范围内的缓存，并标明待更新；无可用缓存时提示暂不可用。天气和空气质量缓存均不包含在工时备份中。旧版天气缓存升级后会尝试补取湿度，离线时仍可显示原有温度。

湿度为上游提供的日平均相对湿度，格内四舍五入为整数百分比，悬停保留原值。AQI 明确使用**美标**，取杭州时间当天 00:00–23:00 共 24 个有效小时预报中的最大值，四舍五入为整数后显示对应等级，不等同于国内 AQI 或监测站实测值。空气质量通常仅覆盖约 5 天，末日可能不完整；不足完整一天、空值或异常数据均显示“暂无”，不会用部分小时值冒充全天最高值。

天气数据来自 [Open-Meteo](https://open-meteo.com/)，空气质量来自 [CAMS](https://atmosphere.copernicus.eu/) 经由 [Open-Meteo Air Quality API](https://open-meteo.com/en/docs/air-quality-api) 提供的模型预报。当前接入其公开接口，适用于其条款允许的非商业使用；商业使用请按 [服务方案](https://open-meteo.com/en/pricing) 配置相应服务。
...
仓库已包含新版界面的构建产物，正常运行只需上述 Python 服务，无需启动 Node、Vite 或 FastAPI。可用 `WORKHOURS_DB_PATH` 指定数据库文件路径；升级启动时会自动补建请假、节假日缓存和天气缓存表，保留原有数据。
...
Python 测试使用临时数据库和可注入的节假日、天气响应，不依赖真实网络。浏览器验收和联网获取应另行使用临时数据库验证。
