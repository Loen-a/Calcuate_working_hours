...
**本分支仅维护 PC 端。** 手机界面已独立保存到 `archive/mobile`，暂停更新。新旧两套界面均使用 PC 布局，最小页面宽度为 1024px；更窄的窗口可横向滚动，不切换手机界面。工时计算与数据格式保持不变。

默认打开新版界面（来自 `local-main` 的 React 布局），顶部按钮可切换新旧界面。选择会保存在当前浏览器中，正在查看的日期随切换保留。两套界面共用 Flask、同一个 SQLite 数据库和原主分支的 Python 工时计算。

新版 PC 界面使用旧界面的 `Segoe UI / Microsoft YaHei` 字体及对应字号层级。桌面样式独立放在 `frontend/src/desktop.css`。

PC 顶部主题按钮依次切换“冷色 → 青绿 → 经典绿”。经典绿采用旧版浅灰绿背景、白色卡片、绿色主按钮和深绿预测区，并随备份保存。窗口缩小时仍保留所选 PC 主题。
