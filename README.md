...
新版 PC 日历显示**杭州（浙江）**从今天起、含今天的 7 天预报，按杭州时间（UTC+8）对应日期。宽格显示天气图标和最低／最高摄氏温度；较窄的格子保留图标，悬停可查看天气描述和温度。过去日期和预报范围外不显示天气。手机和旧版界面保持原样。

天气独立加载，失败不影响打卡、请假或工时计算。服务端缓存一小时，杭州时间换日后重新获取；页面在前台每小时及换日时刷新，重新回到页面时检查是否需要更新。更新失败时仅显示仍在预报范围内的缓存，并标明待更新；无可用缓存时提示暂不可用。天气缓存不包含在工时备份中。

数据来自 [Open-Meteo](https://open-meteo.com/)。当前接入其公开接口，适用于其条款允许的非商业使用；商业使用请按 [服务方案](https://open-meteo.com/en/pricing) 配置相应服务。
