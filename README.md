...
一个个人工作小时统计工具，单页面 Web 应用。数据存本地浏览器（`localStorage`），无需后端，产物为单个 HTML 文件（`dist/index.html` ≈ 590 KB），双击即可打开。
...
- **手动导出 / 导入**（`Header` 右上）：导出 = 下载 `workhours-YYYY-MM-DD.json`；导入 = 选 JSON 覆盖当前 entries（有数据时先 `confirm`）。浏览器**不能**静默写到指定目录——把 Chrome 下载位置改到项目下 `data/` 文件夹可以"默认落盘"。详见 `CLAUDE.md`。
...
npm run build    # 产物：dist/index.html（双击即开）
```
...
变量定义在 `src/index.css` 的 `@theme`，**所有业务组件通过工具类（`bg-paper` / `text-ink` / `border-rule` / `text-navy` 等）引用 `var(--color-*)`**——所以主题切换只需覆盖 CSS 变量，组件零改动。
...
**数据存浏览器、不在文件夹里**——`dist/index.html` 双击打开时，localStorage 按 `file://` 路径 + 浏览器 profile 锁。**挪文件夹、换浏览器、清除浏览数据都会丢**。所以加了手动备份：

- 顶栏 `导出` → 下载 `workhours-YYYY-MM-DD.json`（建议存到项目下 `data/` 文件夹）
- 顶栏 `导入` → 选 JSON 覆盖当前 entries（有数据先 `confirm`）
- 浏览器**不能**让网页静默写到指定目录——想把"默认到 data/"，在 Chrome 设置 → 下载 → 位置改到 `data/` 文件夹（一次性）。详见 `CLAUDE.md`。
